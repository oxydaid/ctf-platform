...
<body class="min-h-screen bg-zinc-100 dark:bg-zinc-800">
    <flux:header sticky class="border-b border-zinc-200 bg-white dark:border-zinc-700 dark:bg-zinc-900">
        ...
        <flux:navbar class="-mb-px max-lg:hidden">
            ...
            <flux:navbar.item icon="goal" :href="route('challenges')" :current="request()->routeIs('challenges')"
                wire:navigate>
                {{ __('Challenges') }}
            </flux:navbar.item>
            <flux:navbar.item icon="medal" :href="route('leaderboards')" :current="request()->routeIs('leaderboards')"
                wire:navigate>
                {{ __('Leaderboards') }}
            </flux:navbar.item>
        </flux:navbar>
    ...
    <flux:sidebar stashable sticky
        ...
        <flux:navlist variant="outline">
            <flux:navlist.group :heading="__('Dashboard')">
                <flux:navlist.item icon="layout-grid" :href="route('dashboard')"
                    :current="request()->routeIs('dashboard')" wire:navigate>
                    {{ __('Dashboard') }}
                </flux:navlist.item>
            </flux:navlist.group>
            <flux:navlist.group :heading="__('CTF')">
                <flux:navlist.item icon="layout-grid" :href="route('challenges')"
                    :current="request()->routeIs('challenges')" wire:navigate>
                    {{ __('Challenges') }}
                </flux:navlist.item>
                <flux:navlist.item icon="layout-grid" :href="route('leaderboards')"
                    :current="request()->routeIs('leaderboards')" wire:navigate>
                    {{ __('Leaderboards') }}
                </flux:navlist.item>
            </flux:navlist.group>
        </flux:navlist>
